...
В данной лабораторной работе рассматривается реализация алгоритма редукции (суммирования) одномерного массива данных на графическом процессоре с использованием технологии CUDA. Актуальность темы обусловлена фундаментальной ролью операции редукции в параллельных вычислениях: она является ключевым этапом во многих алгоритмах машинного обучения, физической симуляции, обработки больших данных и численного анализа, где требуется агрегирование результатов, полученных множеством потоков.

Целью лабораторной работы является изучение принципов организации эффективных параллельных вычислений при выполнении операции суммирования, а также освоение методов оптимизации доступа к памяти GPU. В работе требуется реализовать улучшенное ядро редукции, использующее разделяемую память для минимизации обращений к глобальной памяти и применения паттерна «дерево свёртки». Такой подход позволяет значительно снизить задержки памяти и избежать конфликтов банков, что критически важно для достижения высокой производительности на архитектуре CUDA. Кроме того, предусмотрена обработка списков произвольного размера с корректным учетом граничных условий и финальным подсчетом суммы на центральном процессоре (CPU).

Особенностью рассматриваемой задачи является необходимость балансировки нагрузки между блоками нитей и оптимизация этапов синхронизации внутри блока. Реализация должна обеспечивать обработку входных данных объемом до 2048×65535 элементов за один запуск ядра, используя нейтральные элементы для заполнения незаполненных ячеек разделяемой памяти в последнем блоке. Важным аспектом является сравнение количества операций чтения/записи в глобальную память в базовой и оптимизированной версиях алгоритма, а также анализ влияния структуры дерева редукции на общее количество выполняемых инструкций.

Выполнение лабораторной работы направлено на закрепление навыков разработки высокопроизводительных CUDA-приложений, глубокое понимание иерархии памяти графического ускорителя и механизмов синхронизации потоков. Полученные результаты позволяют оценить эффективность применения стратегий оптимизации памяти и выявить потенциальные узкие места при реализации параллельных алгоритмов сведения данных.

\section{Цель работы}

Получение навыков выполнения редукции одномерного списка.

\section{Задание}

Реализовать программу, выполняющую редукцию одномерного списка с помощью CUDA.

\section{Ход работы}

В листинге ниже приведено содержание файла rvs\_laboratories\_11/\-rvs\_lab\-orat\-ories\_11.cu, который содержит код, выполняющий редукцию одномерного списка с помощью CUDA (лист. \ref{lst:main}).

\begin{code}    
\caption{Содержание rvs\_laboratories\_11/rvs\_laboratories\_11.cu\label{lst:main}}
\inputminted{c++}{../rvs_laboratories_11/rvs_laboratories_11.cu}
\end{code}
...
Как можно видеть, вычисление редукции было выполнено корректно и успешно.

\sectionc{Заключение}

Таким образом, в ходе выполнения данной лабораторной работы были успешно получены навыки выполнения редукции одномерного списка. На практике была реализована программа, выполняющая редукцию одномерного списка с помощью CUDA.
